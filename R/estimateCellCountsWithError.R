...
#'
#' This is an adaptation of estimateCellCounts() from the minfi R package to include the
...
#' @param cellTypes Which cell types, from the reference object, should be we use for the deconvolution? See details.
#' @param referencePlatform The platform for the reference dataset; if the input rgSet belongs to another platform, it will be converted using convertArray.
...
#' @return A matrix with the estimated proportion of cell types across all sites, CETYGO and number of
...
estimateCellCountsWithError <- function(rgSet, compositeCellType = "Blood",
    ...
    if ((compositeCellType == "CordBlood") && (!"nRBC" %in% cellTypes)) {
        message("[estimateCellCountsWithError] Consider including 'nRBC' in argument 'cellTypes' for cord blood estimation.\n")
    }
